...
export function WeatherSummaryCard({ beaufortNumber, summary }: { beaufortNumber: number | null; summary: WeatherSnapshot }) {
  ...
  return (
    <article className="card card-pad weather-card">
      <div className="card-head"><div><h3>Thời tiết hiện tại</h3><p>Cập nhật gần nhất cho Cảng Tiên Sa</p></div></div>
      <div className="weather-grid">{metrics.map(([label, value]) => <div className="weather-metric" key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
    </article>
  );
}
